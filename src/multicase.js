...
function generateStateText(state){
    ...
    return `<div class="data_state" id="state_${state.toLowerCase().replaceAll(" ", "_")}">
         ...
         <div class="form-group col-lg-4 col-sm-2">
            ...
            class="btn btn-custom-light state" onclick = removeState('${state.toLowerCase().replaceAll(" ", "_")}') style="margin-top : 0;">
            Hapus state </button>
         </div>
       </div>
       </div>`
...
function changeButton(){
    button.className = button.className.replaceAll(/col-sm-6/g, "col-sm-3");
...
function returnToDefault(){
    button.className = button.className.replaceAll(/col-sm-3/g, "col-sm-6");
...
function toggleButton(){
    ...
    if(button.className.match(/col-sm-6/g)){
        changeButton();
    } else if(button.className.match(/col-sm-3/g)){
        returnToDefault();
    }
}
...
//Kalau jumlahnya tidak sama dengan jumlah seluruh soal
    ...
    $("#keterangan-nilai").css('display', 'block');
